//----------------------------
// Dialogues after Ellhimar's death / kidnapping
//----------------------------
...
SAY ~...und dann sind die Githyanki durch den Spiegel getreten? So wissen wir wenigstens, dass diese Kreaturen mit dem Bösen, das unsere Stadt so quält, im Bunde stecken! Sagt, <CHARNAME>, was wollte Euch Ellhimar zeigen?~
IF ~~ THEN REPLY ~Bevor er mir seine Entdeckung zeigen konnte, kamen die Githyanki. Das Letzte, was ich erkennen konnte, war ein großer Silberblock in einer anderen Zwergenstadt.~ GOTO torglor_01
END

	IF ~~ THEN BEGIN torglor_01
	SAY ~Ein Silberblock? In einer anderen Stadt? Das könnte Torglor sein, oder was meint Ihr, Vronia?~
	IF ~~ THEN EXTERN ~AC#VRON7~ torglor_02 
	END

IF ~~ THEN BEGIN bettargh_give_book_astral 
  SAY ~Ich möchte noch etwas anmerken. Es geht um das Buch, welches Mith Barak von dieser Bibliothek der Oberfläche - Kerzenburg - haben wollte.~
	...
	SAY ~Beldas war doch auf der Suche nach diesem Buch Oberfläche. Habt Ihr es noch?~
	...
	SAY ~Ihr hattet mir doch eine Abschrift des Buches gegeben, welches Beldas an der Oberfläche erworben hatte: "Die Astrale See, ein Reisebericht in das Reich aus Silber von Maskyr dem Einäugigen". König Mith Barak wollte es wohl unbedingt haben.~
	IF ~~ THEN REPLY ~Habt Ihr herausgefunden, warum Mith Barak es wollte?~ GOTO astral_book_01
	END
	
			IF ~~ THEN BEGIN astral_book_01
			SAY ~König Mith Barak war seit längerer Zeit sehr interessiert an Berichten über die Astralebene. Unter dem Eindruck, dass die Githyanki von dieser Ebene stammen und nun in unsere Stadt eingedrungen sind, wirft das ein völlig neues Licht auf Mith Baraks Nachforschungen.~
			IF ~~ THEN REPLY ~Was ist die Astralebene?~ GOTO whats_astral_plane
			IF ~~ THEN REPLY ~Ihr denkt, Mith Barak war den Githyanki bereits auf der Spur?~ GOTO mithbarak_gith
			END
			
					IF ~~ THEN BEGIN mithbarak_gith
					SAY ~Das wäre möglich. Allerdings werden Githyanki in diesem Buch nicht erwähnt. Es geht voll und ganz um die Beschaffenheit der Astralebene.~
					IF ~~ THEN GOTO whats_astral_plane
					END
						...
						END
							...
							END
								...
								END
									...
									SAY ~Davon bin ich mittlerweile fest überzeugt. Und dabei kommen wieder die Githyanki ins Spiel, denn irgendwie müssen sie aus ihrer Heimat - der Astralebene - hier zu uns gelangt sein.~									
									IF ~~ THEN REPLY ~Ihr glaubt, Mith Barak suchte das Portal, das die Githyanki verwendet haben?~ EXTERN ~AC#GROM7~ gromi_gith_seek_portal
									END

									IF ~~ THEN BEGIN bettargh_search_library
									SAY ~Ich werde in der Bibliothek weitersuchen, ob sich dazu weitere Hinweise finden lassen. Dennoch ist Euer Weg an die Oberfläche der nächste wichtige Schritt, und ich wollte Euch auch nicht zusätzlich verwirren. Ich möchte nur, dass Ihr bei Eurer Suche nach Antworten auf den mysteriösen Schlaf auch diese Mitteilung in Betracht zieht.~
									=
									~Elern erwähnte, dass Ihr von Anthan Diamantklinge das Tagebuch das alten Borthun zu Rate ziehen wolltet. Habt Ihr es bereits erhalten?~									
									IF ~PartyHasItem("AC#BOOKB")~ THEN REPLY ~Ja, ich habe es bei mir.~ GOTO yes_have_borthuns_book
									IF ~!PartyHasItem("AC#BOOKB")~ THEN REPLY ~Nein, noch nicht.~ GOTO nope_have_borthuns_book
									END
																	
									
									IF ~~ THEN BEGIN yes_have_borthuns_book
									SAY ~Ich habe Euch auch eine kleine Niederschrift des Inhaltes von Maskyrs Buch mitgebracht. Wie ich hörte, seid Ihr ja im Besitz von Borthuns Tagebuch. Legt meine Notizen doch einfach dazu, dann findet Ihr sie auch jederzeit wieder.~
									IF ~~ THEN DO ~SetGlobal("AC#BettarghBookAstral","GLOBAL",10)~ EXTERN ~AC#GROM7~ gromi_bye_2_charname_02
									END	
									
									IF ~~ THEN BEGIN nope_have_borthuns_book
...
SAY ~So lasst uns wieder zu unserer eigentlichen Aufgabe zurückkehren und die Hilfe für unseren schlafenden König vorantreiben.~
IF ~~ THEN REPLY ~Könnten wir nicht einfach in dieses Torglor gehen und den Githyanki eine Abreibung verpassen?~ GOTO githyanki_slaughter
IF ~~ THEN REPLY ~Gute Idee.~ EXTERN ~AC#BETT7~ bettargh_give_book_astral
END

	IF ~~ THEN BEGIN githyanki_slaughter
	SAY ~Wir wissen ja noch nicht einmal, ob sie etwas mit dem Zustand unseres Königs zu tun haben. Mein Gefühl sagt mir, dass wir woanders weitersuchen müssen - zumindest fürs Erste.~
	IF ~~ THEN EXTERN ~AC#BETT7~ bettargh_give_book_astral
	END

IF ~~ THEN BEGIN gromi_gith_seek_portal
SAY ~Das ist ein guter Hinweis. Bisher sind das aber alles nur Mutmaßungen. Wir sollten deshalb umso rascher nach der Ursache für den Schlaf unseres geliebten Monarchen suchen.~
IF ~~ THEN EXTERN ~AC#BETT7~ bettargh_search_library
END
...
EraseJournalEntry(@62200)
EraseJournalEntry(@62201)
ActionOverride("AC#BETT7",EscapeArea())
ActionOverride("AC#VRON7",EscapeArea())
...
SAY ~Das wäre eine Möglichkeit, ja.~
IF ~~ THEN EXTERN ~AC#BRES7~ chain_bresk_gith
END
